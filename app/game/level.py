...
class Level:
    def __init__(self, level: str):
        ...
        self.obj_dict = {"terrain": self.terrain, "fruit": self.fruit}

    def load(
        self, camera: Camera, target: Union["Player", "EditorCamera"]
    ) -> list[Entity]:
        self.camera = camera
        ...
        for dic in [self.terrain, self.fruit]:
            for _, v in dic.items():
                if "var" in v:
                    self.objs.append(
                        Terrain(
                            v["name"], v["var"], pygame.Vector2(v["pos"]), self.camera
                        )
                    )
                else:
                    self.objs.append(
    ...
    def get_top_left(self) -> pygame.Vector2:
        ...
        return pygame.Vector2(min_x * RECT_W, min_y * RECT_H)
    ...
    def get_bottom_right(self) -> pygame.Vector2:
        ...
        return pygame.Vector2((max_x + 1) * RECT_W, (max_y + 1) * RECT_H)
    ...
    def get_player_pos(self) -> pygame.Vector2:
        return pygame.Vector2(self.player["pos"][0], self.player["pos"][1])

    def get_player_name(self) -> str:
        return self.player["name"]

    def check_obj_dict(self, x: int, y: int) -> Union[str, None]:
        key = f"{x};{y}"
    ...
    def add_block(
        ...
        if block_type == "terrain":
            ...
            self.fruit[f"{x};{y}"] = {"name": name or "pineapple", "pos": [x, y]}
        else:
