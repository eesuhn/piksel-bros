...
class Editor(Game):
    ...
    def _load_previews(self) -> None:
        ...
        fruit_sheet = load_sprites_sheet(
            "assets/sprites/fruits", FRUIT_W, FRUIT_H, scale=(2 * self.preview_scale)
        )
    ...
    def load_level(self) -> None:
        ...
        self.editor_camera = EditorCamera(
            int(camera_pos.x), int(camera_pos.y), self.camera
        )
    ...
    def loop(self) -> None:
        ...
        self.camera.update(display=self.display, top_left=pygame.Vector2((0, 0)))
    ...
    def check_events(self) -> None:
        ...
        for event in self.events:
            if event.type == pygame.VIDEORESIZE:
                self.screen = pygame.display.set_mode((event.w, event.h), SCR_FLAGS)
